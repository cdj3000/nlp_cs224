...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_palceholder=tf.placeholder(tf.int32,[None,self.config.n_features],name=None)
        self.labels_placeholder=tf.placeholder(tf.float32,[None, self.config.n_classes],name=None)
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        """
        ### YOUR CODE HERE
        feed_dict={self.input_palceholder:inputs_batch,
    ...
    def add_embedding(self):
        ...
        embedding_lookup=tf.nn.embedding_lookup(embedding,self.input_palceholder)

        embeddings=tf.reshape(embedding_lookup,[-1,self.config.n_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        self.w=tf.Variable(initializer([self.config.n_features*self.config.embed_size,self.config.hidden_size]))
        
        
        self.u=tf.Variable(initializer([self.config.hidden_size, self.config.n_classes]))
        
        
        self.b1=tf.zeros([self.config.hidden_size])
        
        
        self.b2=tf.zeros([self.config.n_classes])
        
        
        
        
        
        h1=tf.matmul(x,self.w)+self.b1
        
        h1_relu=tf.nn.relu(h1)
        h1_dropout=tf.nn.dropout(h1_relu,self.dropout_placeholder)
        
        pred= tf.matmul(h1_dropout,self.u)+self.b2
        ### END YOUR CODE
        return pred
