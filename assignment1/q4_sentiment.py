...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    word_vectors=wordVectors[word_indices]
    sentVector=np.sum(word_vectors,0)/(word_vectors.shape[0])
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    for result in results:
        if result['dev']>dev_temp:
            bestResult=result
    
    assert bestResult==max(results, key = lambda x: x['dev']),'somthing going wrong'
