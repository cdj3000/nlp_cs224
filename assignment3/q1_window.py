...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        sentence_token_list_with_start_and_end=sentence.copy()
        for index in range(window_size):
            sentence_token_list_with_start_and_end.insert(0,start)
            sentence_token_list_with_start_and_end.append(end)
            
        for index in range(len(sentence)):
            temp= sentence_token_list_with_start_and_end[index: index+2*window_size + 1]
            
            windowed_data.append(tuple([temp,labels[index]]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings=tf.nn.embedding_lookup(embedding_table,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        w=tf.get_variable(shape=[self.config.n_window_features*self.config.embed_size,self.config.hidden_size],initializer=tf.contrib.layers.xavier_initalizwer(True))
        u=tf.get_variable(shape=[self.config.hidden_size,self.config.n_classes],initializer=tf.contrib.layers.xavier_initializer(True))
        b1=tf.zeros(shape=[1,self.config.hidden_size,self])
        b2=tf.zeros(shape=[1,self.config.n_classes])
        ht=tf.nn.relu(self.input_placeholder*w+b1)
        ht_drouped= tf.nn.dropout(ht,self.dropout_placeholder)
        pred= ht_drouped*u+b2
    ...
    def add_loss_op(self, pred):
        ...
        loss=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.labels_placeholder, logits=pred)
    ...
    def add_training_op(self, loss):
        ...
        train_op= tf.train.AdamOptimizer(sefl.config.lr).minimize(loss)
